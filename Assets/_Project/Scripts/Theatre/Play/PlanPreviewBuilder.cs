...
namespace Vanquish.Theatre.Play
{
    ...
    /// hardware it'll actually fly as once built.
    /// </summary>
    public static class PlanPreviewBuilder
    {
        public static GameObject Build(Transform parent, DronePlan plan, float scale = 0.6f)
        {
            ...
            if (plan.Category == UnitCategory.Missile)
                BuildMissilePreview(root.transform, plan.AccentColor);
            else
        ...
        private static void BuildMissilePreview(Transform parent, Color accentColor)
        {
            GameObject body = GameObject.CreatePrimitive(PrimitiveType.Capsule);
            body.name = "Body";
            body.transform.SetParent(parent, false);
            body.transform.localRotation = Quaternion.Euler(90f, 0f, 0f);
            body.transform.localScale = new Vector3(0.16f, 0.4f, 0.16f);
